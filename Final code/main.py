...
def Admin():
    if request.method == 'POST':
        ...
        current_otp = sendEmailVerificationRequest(
            receiver=email)  # this function sends otp to the receiver and also returns the same otp for our session storage
...
def face():
    if request.method == 'POST':
        ...
        insertQuery = "INSERT INTO atmtb VALUES ('" + card_number + "')"
        mycursor.execute(insertQuery)
        mydb.commit()
        transform = transforms.Compose([
            transforms.Resize((160, 160)),
